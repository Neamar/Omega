...
class Eleve_ExerciceController extends ExerciceAbstractController
{
		...
		if(isset($_POST['creation-exercice']))
		{
			...
			else
			{
				...
				if(Sql::insert('Exercices', $ToInsert))
				{
					...
					$this->View->setMessage('ok', 'Votre exercice a bien été ajouté. Vous pouvez maintenant ajouter des fichiers.');
	...
	public function annulationActionWd()
	{
		...
		if(isset($_POST['annulation']))
		{
			...
			{
				$ToDispatch = Event::ELEVE_EXERCICE_REFUS;
				$Datas = array(
					'Exercice' => $this->Exercice,
					'Correcteur' => $this->Exercice->getCorrecteur(),
					'Eleve' => $this->getMembre(),
					'prix_correcteur' => $this->Exercice->priceAsked()
				);
			}
